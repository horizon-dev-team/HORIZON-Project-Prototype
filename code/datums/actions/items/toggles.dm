...
/datum/action/item_action/toggle_nv/do_effect(trigger_flags)
	...
	if(stored_cutoffs)
		...
		playsound(goggles, 'sound/items/night_vision_on.ogg', 30, TRUE, -3, mixer_channel = CHANNEL_UI)
	else
		...
		playsound(goggles, 'sound/machines/click.ogg', 30, TRUE, -3, mixer_channel = CHANNEL_UI)
	holder?.update_sight()
