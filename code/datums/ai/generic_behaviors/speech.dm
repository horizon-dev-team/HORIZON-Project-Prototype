...
/datum/bt_node/ai_behavior/random_speech/perform(seconds_per_tick, datum/ai_controller/controller)
	...
	if(roll <= audible)
		...
		if(length(sound))
			playsound(pawn, pick(sound), 80, vary = TRUE, pressure_affected = TRUE, ignore_walls = FALSE, mixer_channel = CHANNEL_MOB_EMOTES)
...
/datum/bt_node/ai_behavior/random_speech/proc/speak(mob/living/pawn, datum/ai_controller/controller)
	...
	if(length(sound))
		playsound(pawn, pick(sound), 80, vary = TRUE, mixer_channel = CHANNEL_MOB_EMOTES)
...
/datum/bt_node/ai_behavior/random_speech_blackboard/perform(seconds_per_tick, datum/ai_controller/controller)
	...
	if(roll <= length(emote_hear))
		pawn.manual_emote(pick(emote_hear))
		if(sound_to_play)
			playsound(pawn, sound_to_play, 80, vary = TRUE, pressure_affected = TRUE, ignore_walls = FALSE, mixer_channel = CHANNEL_MOB_EMOTES)
		...
		if(sound_to_play)
			playsound(pawn, sound_to_play, 80, vary = TRUE, mixer_channel = CHANNEL_MOB_EMOTES)
